...
enum StoreDownloadProtocol {
    enum Endpoint: String {
        ...
        var versionKey: String { self == .volumeStore ? "externalVersionId" : "appExtVrsId" }
        var path: String {
    ...
    static func payload(endpoint: Endpoint, appID: Int64, guid: String, version: String?) -> [String: Any] {
        ...
        if let version, !version.isEmpty { result[endpoint.versionKey] = version }
        return result
    }

    static func failureCode(_ response: [String: Any]) -> String {
        let failure = StoreAuthenticationProtocol.string(response["failureType"])
        if !failure.isEmpty { return failure }
        let metrics = response["metrics"] as? [String: Any]
        let code = StoreAuthenticationProtocol.string(metrics?["messageCode"])
        return code == "0" ? "" : code
    }

    static func fallbackReason(_ response: [String: Any]) -> String? {
        let failure = failureCode(response)
        if failure == "5002" { return "failure-5002" }
        guard failure.isEmpty,
              StoreAuthenticationProtocol.string(response["customerMessage"]).isEmpty,
              response["dialog"] == nil, response["action"] == nil
        else { return nil }
        let status = StoreAuthenticationProtocol.string(response["status"])
    ...
    static func fetchWithFallback(
        ...
        onFallback(reason)
        let resolved: String
        if let version, !version.isEmpty { resolved = version }
        else { resolved = try await resolveVersion() }
    ...
    static func summary(_ response: [String: Any]) -> String {
        func numeric(_ value: Any?) -> String {
            let text = StoreAuthenticationProtocol.string(value)
            if text.isEmpty { return "none" }
            return text.count <= 12 && text.allSatisfy({ $0.isASCII && ($0.isNumber || $0 == "-") }) ? text : "other"
        }
        let items: String
        if let list = response["songList"] as? [Any] { items = String(list.count) }
        else { items = response["songList"] == nil ? "missing" : "invalid" }
        let message = !StoreAuthenticationProtocol.string(response["customerMessage"]).isEmpty
    ...
    static func packageItem(_ response: [String: Any], bundleID: String) throws -> [String: Any] {
        let code = failureCode(response)
        let message = StoreAuthenticationProtocol.string(response["customerMessage"])
        if !code.isEmpty { throw StoreDownloadError.rejected(code, message) }
        guard let items = response["songList"] as? [[String: Any]], !items.isEmpty else {
            if !message.isEmpty { throw StoreDownloadError.rejected("", message) }
            if response["dialog"] != nil || response["action"] != nil { throw StoreDownloadError.actionRequired }
            let status = StoreAuthenticationProtocol.string(response["status"])
            if !status.isEmpty, status != "0" { throw StoreDownloadError.rejected(status, "") }
