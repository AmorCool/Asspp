...
actor SignedStoreAuthenticator {
    ...
    func authenticate(email: String, password: String, code: String, guid: String, cookies: [Cookie]) async throws -> ApplePackage.Account {
        ...
        let endpoint = try StoreAuthenticationProtocol.authenticationURL(StoreAuthenticationProtocol.string(value("authenticateAccount")))
        ...
        while protocolAttempt <= 4, redirects <= 3 {
            ...
            if let value = response.value(forHTTPHeaderField: "X-Set-Apple-Store-Front") { storefront = value }
            if let value = response.value(forHTTPHeaderField: "pod") { pod = value }
    ...
    private func sendAuthentication(_ request: URLRequest, signer: SAPContext) async throws -> (Data, HTTPURLResponse) {
        for attempt in 1 ... 3 {
            ...
            signedRequest.setValue(try signer.sign(request.httpBody ?? Data()).base64EncodedString(), forHTTPHeaderField: "X-Apple-ActionSignature")
            let result = try await send(signedRequest)
            if attempt == 3 || !StoreAuthenticationProtocol.retryable(status: result.1.statusCode, data: result.0) { return result }
